...
public class FilmController {
    ...
    private void validateDataFilm(Film film) {
        ...
        if (film.getDuration() == null || film.getDuration().isBlank()) {
            throw new ValidationException("Продолжительность не может быть пустой!");
        }

        try {
            LocalTime.parse(film.getDuration());
        } catch (Exception e) {
            throw new ValidationException("Некорректный ввод продолжительности (HH:mm)");
        }
    }
}
